    ...
    try:
        ...
        if _official is not None:
            try:
                ...
                # V164: publicar la geometría que realmente muestra el visor para
                # que el ZIP maestro no vuelva a exportar la piel plantarflexionada.
                st.session_state["v164_corrected_skin_export"]={
                    "vertices":np.asarray(Vviz,dtype=np.float32),
                    "faces":np.asarray(F,dtype=np.int32),
                    "qc":_skin_qc_v163,
                    "gender":str(_official.get("gender",mesh_seq.get("skel_gender","unknown"))),
                    "frames":int(len(Vviz)),
                }
