...
def _continuous_foot_frames(J,nm,side):
    """Rotation-minimizing anatomical foot frames over the complete sequence.

    The heel->toe axis is sign-continuous. The transverse axis is parallel-
    transported from the previous frame, eliminating the equivalent 180-degree
    solutions produced by rebuilding the basis independently per frame.
    """
    origins=[]; bases=[]; prev_fwd=None; prev_right=None
    for t in range(len(J)):
        right_b,ant_b,up_b,_=_body_frame_target(J,nm,t)
        ankle=np.asarray(J[t,nm[f"talus_{side}"]],float)
        heel=np.asarray(J[t,nm[f"calcn_{side}"]],float)
        toe=np.asarray(J[t,nm[f"toes_{side}"]],float)
        fwd=_unit(toe-heel,ant_b)
        if prev_fwd is not None and np.dot(fwd,prev_fwd)<0:
            fwd=-fwd
        seed=right_b if prev_right is None else prev_right
        right=seed-fwd*np.dot(seed,fwd)
        if np.linalg.norm(right)<1e-7:
            right=right_b-fwd*np.dot(right_b,fwd)
        right=_unit(right,right_b)
        dorsal=_unit(np.cross(right,fwd),up_b)
        right=_unit(np.cross(fwd,dorsal),right)
        if prev_right is None and np.dot(dorsal,up_b)<0:
            right=-right; dorsal=-dorsal
        B=np.stack([right,fwd,dorsal],axis=1)
        if np.linalg.det(B)<0.999:
            raise RuntimeError("QC pie bloqueado: frame temporal no es rotación propia")
        origins.append(ankle); bases.append(B)
        prev_fwd=fwd; prev_right=right
    return np.asarray(origins,float),np.asarray(bases,float)


def _rotation_jump_degrees(B):
    vals=[]
    for t in range(1,len(B)):
        R=B[t]@B[t-1].T
        vals.append(float(np.degrees(np.arccos(np.clip((np.trace(R)-1.0)/2.0,-1,1)))))
...
def neutralize_skel_skin_with_official_weights(vertices,faces,joints,joint_names,model_data):
    """V163 viewer correction using the official SKEL LBS weights.

    Requires exact template topology identity. Interior foot vertices move
    rigidly; the ankle transition follows the native SKEL blend weights.
    """
    V=np.asarray(vertices,np.float32); F=np.asarray(faces,np.int32); J=np.asarray(joints,np.float32)
    template_f=np.asarray(model_data.get("skin_template_f"),np.int32)
    if V.ndim!=3 or V.shape[1]!=6890 or F.shape!=template_f.shape or not np.array_equal(F,template_f):
        raise RuntimeError("QC skinning bloqueado: topología NPZ distinta de la plantilla SKEL oficial")
    bone_names=[str(x) for x in model_data.get("bone_names",[])]
    if len(bone_names)!=24:
        raise RuntimeError("QC skinning bloqueado: mapa óseo SKEL oficial incompatible")
    W=model_data.get("skin_weights")
    if W is None or tuple(W.shape)!=(6890,24):
        raise RuntimeError("QC skinning bloqueado: pesos SKEL ausentes o incompatibles")
    W=W.tocsr() if hasattr(W,"tocsr") else np.asarray(W,float)
    nm={str(n):i for i,n in enumerate(joint_names)}
    out=V.copy(); qc={"method":"official_SKEL_LBS_weights","topology_exact":True,"sides":{}}
    for side in ("r","l"):
        cols=[bone_names.index(f"{b}_{side}") for b in ("talus","calcn","toes")]
        wf=np.asarray(W[:,cols].sum(axis=1)).reshape(-1).astype(np.float32)
        wf=np.clip(wf,0.0,1.0)
        offset=_neutral_foot_offset(J,nm,side)
        O,B=_continuous_foot_frames(J,nm,side)
        for t in range(len(J)):
            R=_axis_rotation(B[t,:,0],offset)
            rigid=O[t]+(V[t]-O[t])@R.T
            out[t]+=(wf[:,None]*(rigid-V[t])).astype(np.float32)
        qc["sides"][side]={
            "offset_deg":float(np.degrees(offset)),
            "fully_rigid_vertices":int(np.sum(wf>0.98)),
            "transition_vertices":int(np.sum((wf>0.01)&(wf<0.98))),
            "unaffected_vertices":int(np.sum(wf<=0.01)),
            "max_weight":float(np.max(wf)),
        }
    if not np.isfinite(out).all():
        raise RuntimeError("QC skinning bloqueado: vértices no finitos")
    qc["pass"]=True
    return out,qc
...
def _register_denver_bones_frame0(master,J,nm,man=None,return_qc=False,axis_map=None):
    ...
    for ip,name in enumerate(master["part_names"]):
        ...
        if seg=="foot" and fs is not None:
            ...
            _,FBs=_continuous_foot_frames(J,nm,target_side); FB=FBs[0]
    ...
    # Patella fail-safe: correct only the patellar mesh, never femur/trochanter.
    patella_corrected={}; patella_margin={}
    for side in ("r","l"):
        parts=[ip for ip,name in enumerate(master["part_names"])
               if _norm(name)=="patella" and target_by_part[ip][0]==side]
        if not parts:
            raise RuntimeError(f"QC anatómico bloqueado: rótula {side.upper()} no identificable")
        knee=np.asarray(J[0,nm[f"tibia_{side}"]],float)
        ids=np.concatenate([np.where(master["part_index"]==ip)[0] for ip in parts])
        margin=float(np.dot(np.mean(out[ids],axis=0)-knee,display_ant))
        corrected=False
        if margin<=0:
            for ip in parts:
                idx=np.where(master["part_index"]==ip)[0]
                rel=out[idx]-knee
                out[idx]=out[idx]-2.0*(rel@display_ant)[:,None]*display_ant[None,:]
            corrected=True
            margin=float(np.dot(np.mean(out[ids],axis=0)-knee,display_ant))
        if margin<=1e-5:
            raise RuntimeError(f"QC anatómico bloqueado: rótula {side.upper()} no queda anterior")
        patella_corrected[side]=corrected; patella_margin[side]=margin
    qc={"version":"162-foot-temporal-continuity","pelvis_ap_corrected":pelvis_flipped,
        ...
        "patella_ap_corrected":patella_corrected,
        "patella_anterior_margin":patella_margin,
        "pass":True}
...
def build_visible_human_bone_sequence(man,joints,joint_names,skin_sequence,max_faces_each=70,axis_map=None):
    ...
    foot_frames={s:_continuous_foot_frames(J,nm,s) for s in ("r","l")}
    frames=[V0]
    for t in range(1,len(J)):
        Vt=np.zeros_like(V0)
        for ip,name in enumerate(master["part_names"]):
            ...
            try:
                if seg=="foot":
                    O,B=foot_frames[side]; o0,ot=O[0],O[t]; R=B[t]@B[0].T
                else:
...
def _segment_affines_compact(J,nm):
    ...
    for t in range(len(J)):
        for si,(side,seg) in enumerate(keys):
            if seg=="foot":
                O,B=foot_frames[side]; o0,B0=O[0],B[0]; ot,Bt=O[t],B[t]
            else:
                o0,B0,_=_semantic_target_frame(J,nm,0,side,seg)
                ot,Bt,_=_semantic_target_frame(J,nm,t,side,seg)
            R=Bt@B0.T
...
def build_visible_human_compact_payload(man,joints,joint_names,max_bone_faces_each=700,max_muscle_faces_each=300,axis_map=None):
    ...
    temporal={}
    for side in ("r","l"):
        _,_B=_continuous_foot_frames(J,nm,side)
        _j=_rotation_jump_degrees(_B)
        temporal[side]={"max_frame_jump_deg":float(np.max(_j)) if len(_j) else 0.0,
                        "p95_frame_jump_deg":float(np.percentile(_j,95)) if len(_j) else 0.0}
        if len(_j) and float(np.max(_j))>35.0:
            raise RuntimeError(f"QC temporal bloqueado: salto del pie {side.upper()} >35 grados/frame")
    anatomy_qc["foot_temporal_continuity"]=temporal
